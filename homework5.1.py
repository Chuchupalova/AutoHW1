...
search_criteria = (2017, 1.6, 36000)  # year >= 2017, engine >= 1.6, price <= 36000
#фильтр авто
filtered_cars = [
    (name, data)
    for name, data in car_data.items()
    if data[1] >= search_criteria and data[2] >= search_criteria[1] and data[4] <= search_criteria[2]
]
#cортировка возрост. по ценам
sorted_cars = sorted(filtered_cars, key=lambda x: x[1] [4])
#вывод 5 машин
if sorted_cars:
    for name, data in sorted_cars [:5]:
        color, year, engine, car_type, price = data
        print(f"{color}: {year} {engine} {car_type} {price} {name}")
    else:
        print("No cars found")